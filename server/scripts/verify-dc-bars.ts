...
async function verifyDCBars() {
  console.log("Starting Washington DC kava bars verification...\n");

  try {
    // Get all DC kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE (address ILIKE '%washington%' AND (address ILIKE '%, dc%' OR address ILIKE '%, d.c.%'))
      ORDER BY name ASC;
    `);
