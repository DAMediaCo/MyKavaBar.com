...
async function verifyNCBars() {
  console.log("Starting North Carolina kava bars verification...\n");

  try {
    // Get all NC kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address ILIKE '%north carolina%' OR address ILIKE '%, nc%'
      ORDER BY name ASC;
    `);
