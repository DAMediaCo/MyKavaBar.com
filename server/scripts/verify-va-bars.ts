...
async function verifyVABars() {
  console.log("Starting Virginia kava bars verification...\n");

  try {
    // Get all VA kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address ILIKE '%virginia%' OR address ILIKE '%, va%'
      ORDER BY name ASC;
    `);
