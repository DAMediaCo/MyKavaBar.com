...
async function verifyWVBars() {
  console.log("Starting West Virginia kava bars verification...\n");

  try {
    // Get all WV kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address ILIKE '%west virginia%' OR address ILIKE '%, wv%'
      ORDER BY name ASC;
    `);
