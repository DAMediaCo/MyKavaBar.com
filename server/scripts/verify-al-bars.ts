...
async function verifyALBars() {
  console.log("Starting Alabama kava bars verification...\n");

  try {
    // Get all AL kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address ILIKE '%alabama%' OR address ILIKE '%, al%'
      ORDER BY name ASC;
    `);
