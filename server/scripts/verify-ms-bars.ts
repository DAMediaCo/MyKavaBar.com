...
async function verifyMSBars() {
  console.log("Starting Mississippi kava bars verification...\n");

  try {
    // Get all MS kava bars
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address ILIKE '%mississippi%' OR address ILIKE '%, ms%'
      ORDER BY name ASC;
    `);
