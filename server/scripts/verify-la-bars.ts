...
async function verifyLABars() {
  console.log("Starting Louisiana kava bars verification...\n");

  try {
    // Get all LA kava bars with more precise filtering
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address LIKE '%Louisiana%' 
         OR address LIKE '%LA %'
         OR address LIKE '%LA,'
         OR address LIKE '%, LA'
         OR address LIKE '%, LA.%'
         AND NOT (
             address ILIKE '%lake%'
             OR address ILIKE '%las vegas%'
             OR address ILIKE '%lantana%'
         )
      ORDER BY name ASC;
    `);
