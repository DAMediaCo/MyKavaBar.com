...
async function verifyIDBars() {
  console.log("Starting Idaho kava bars verification...\n");

  try {
    // Get all ID kava bars with precise filtering
    const bars = await db.execute<{ id: number, name: string, address: string, place_id: string }>(sql`
      SELECT id, name, address, place_id 
      FROM kava_bars 
      WHERE address LIKE '%Idaho%' 
         OR address LIKE '%ID %'
         OR address LIKE '%ID,'
         OR address LIKE '%, ID'
         OR address LIKE '%, ID.%'
