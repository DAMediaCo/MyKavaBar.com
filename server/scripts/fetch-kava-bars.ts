...
const state_cities: Record<string, string[]> = {
  "Florida": ["Miami", "Orlando", "Tampa", "Jacksonville", "Fort Lauderdale", 
              "West Palm Beach", "Gainesville", "Tallahassee", "Pensacola"],
...
async function searchKavaBars(city: string, state: string): Promise<KavaBarDetails[]> {
      ...
      const details = await googleMapsClient.placeDetails({
        params: {
          ...
          fields: ['name', 'formatted_address', 'formatted_phone_number',
                  'opening_hours', 'geometry', 'rating', 'business_status'],
          key: API_KEY
        }
      });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  fetchKavaBars(state)
    .then(allKavaBars => {
      ...
      const csvContent = [
        ['name', 'address', 'phone', 'hours', 'place_id', 'lat', 'lng', 'rating', 'business_status'],
        ...allKavaBars.map(bar => [
          bar.name,
          bar.address,
          bar.phone,
