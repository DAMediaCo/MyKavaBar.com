...
import { z } from "zod";
import { isPhoneVerifiedMiddleware } from "../middleware/auth";
  ...
  async (req: Express.Request, res) => {
    try {
      ...
      if (existingReview) {
        return res
          .status(400)
          .json({ message: "You have already reviewed this bar" });
      }
...
router.put(
  ...
  async (req: Express.Request, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to update review" });
    }
  },
);
...
router.delete(
  ...
  async (req: Express.Request, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to delete review" });
    }
  },
);
