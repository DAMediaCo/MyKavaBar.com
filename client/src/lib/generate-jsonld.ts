/**
 * Generates JSON-LD structured data for a kava bar listing
 * Schema: https://schema.org/BarOrPub
 */
...
export interface JsonLdBarOrPub {
  "@context": string;
  "@type": string;
  "@id": string;
  name: string;
...
/**
 * Creates a URL-friendly slug from a bar name
 */
...
/**
 * Generates a JSON-LD BarOrPub schema object for a kava bar listing
...
export function generateBarJsonLd(
  listing: BarListing,
  baseUrl: string = "https://mykavabar.com"
): JsonLdBarOrPub {
  const slug = createSlug(listing.name);
  const addressParts = listing.address ? parseAddress(listing.address) : null;
  
  const jsonLd: JsonLdBarOrPub = {
    "@context": "https://schema.org",
    "@type": "BarOrPub",
    "@id": `${baseUrl}/kava-bars/${slug}-${listing.id}#business`,
    "name": listing.name,
    "url": `${baseUrl}/kava-bars/${listing.id}`,
  ...
  if (listing.hours?.periods && listing.hours.periods.length > 0) {
    const hoursSpec: JsonLdBarOrPub["openingHoursSpecification"] = [];
