...
export default function MapView({
  ...
  // Force resize on window changes and add tile reload capability
  useEffect(() => {
    const handleResize = () => {
      if (mapRef.current) {
    ...
    // Force reload all tiles every 30 seconds if there's an error
    const intervalId = tileLoadError ? 
      setInterval(() => {
        console.log("Forcing tile reload due to previous errors");
        if (mapRef.current) {
          // Try a different provider
          setActiveTileProvider(prev => (prev + 1) % tileProviders.length);

          // Force invalidate tiles and redraw
