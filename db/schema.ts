...
export const userRole = pgEnum('user_role', ['regular_user', 'kavatender', 'bar_owner', 'admin']);
...
export const bannedPhoneNumbers = pgTable("banned_phone_numbers", {
  ...
  reason: text("reason"),
  bannedBy: integer("banned_by").references(() => users.id).notNull(),
...
export const passwordResetTokens = pgTable("password_reset_tokens", {
  ...
  phoneVerificationId: integer("phone_verification_id")
    .references(() => phoneVerificationCodes.id, { onDelete: "set null" }),
  expiresAt: timestamp("expires_at").notNull(),
...
export const kavaBars = pgTable("kava_bars", {
  ...
  dataCompletenessScore: decimal("data_completeness_score", { precision: 3, scale: 2 }).default("0.00"),
...
export const verificationRequests = pgTable("verification_requests", {
  id: serial("id").primaryKey(),
  barId: integer("bar_id").references(() => kavaBars.id, { onDelete: "cascade" }).notNull(),
...
export const sponsoredListings = pgTable("sponsored_listings", {
  id: serial("id").primaryKey(),
  barId: integer("bar_id").references(() => kavaBars.id, { onDelete: "cascade" }),
...
export const crowdDensityLevel = pgEnum('crowd_density_level', ['low', 'medium', 'high', 'very_high']);
...
export const checkIns = pgTable("check_ins", {
  id: serial("id").primaryKey(),
  barStaffId: integer("bar_staff_id").references(() => barStaff.id, { onDelete: "cascade" }).notNull(),
...
export const notificationType = pgEnum('notification_type', ['review', 'photo']);

// Add notification preferences table
export const barOwnerNotificationPreferences = pgTable("bar_owner_notification_preferences", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
...
export const phoneVerificationCodeRelations = relations(phoneVerificationCodes, ({ one }) => ({
  user: one(users, {
...
export const sponsoredListingRelations = relations(sponsoredListings, ({ one }) => ({
  bar: one(kavaBars, {
...
export const verificationCodeRelations = relations(verificationCodes, ({ one }) => ({
  bar: one(kavaBars, {
...
export const barOwnerNotificationPreferenceRelations = relations(barOwnerNotificationPreferences, ({ one }) => ({
  user: one(users, {
...
export const barOwnerNotificationRelations = relations(barOwnerNotifications, ({ one }) => ({
  owner: one(users, {
...
export type BarOwnerNotificationPreference = typeof barOwnerNotificationPreferences.$inferSelect;
export type InsertBarOwnerNotificationPreference = typeof barOwnerNotificationPreferences.$inferInsert;
...
export type InsertBarOwnerNotification = typeof barOwnerNotifications.$inferInsert;
...
export const selectPhoneVerificationCodeSchema = createSelectSchema(phoneVerificationCodes);
export const insertBannedPhoneNumberSchema = createInsertSchema(bannedPhoneNumbers);
export const selectBannedPhoneNumberSchema = createSelectSchema(bannedPhoneNumbers);
...
export const insertBarOwnerNotificationPreferenceSchema = createInsertSchema(barOwnerNotificationPreferences);
export const selectBarOwnerNotificationPreferenceSchema = createSelectSchema(barOwnerNotificationPreferences);
export const insertNotificationSchema = createInsertSchema(notifications);
export const selectNotificationSchema = createSelectSchema(notifications);
// Add schemas for barOwnerNotifications (after existing schemas)
export const insertBarOwnerNotificationSchema = createInsertSchema(barOwnerNotifications);
export const selectBarOwnerNotificationSchema = createSelectSchema(barOwnerNotifications);